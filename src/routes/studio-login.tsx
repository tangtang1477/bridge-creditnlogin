...
export const Route = createFileRoute("/studio-login")({
  ...
  head: () => ({
    meta: [
      { title: "MovieFlow × Studio · Login Bridge" },
      { name: "description", content: "Link your MovieFlow account with Studio." },
    ],
  }),
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              {tr("login_bridge_title", locale)}
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-white/60">
              {tr("login_bridge_sub", locale)}
            </p>
          </div>
          <Link
            to="/studio-credits"
          ...
          >
            {tr("goto_credits_bridge", locale)}
          </Link>
        </div>

        <div className="mt-8">
          {loading && <div className="h-40 animate-pulse rounded-2xl glass-card" />}
